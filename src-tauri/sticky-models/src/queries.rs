...
pub async fn list_notes<R: Runtime>(
    ...
    let db = connect(app_handle).await?;
    ...
    Ok(items.collect::<rusqlite::Result<Vec<Note>>>()?)
}
...
// Use the current time when the timestamp is zero, else the given one.
