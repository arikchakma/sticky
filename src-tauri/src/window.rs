...
// the window's label. Dragging from the header hands the mouse session
// to the native drag loop, which swallows the mouseup and resets
// WebKit's own click counter, so double clicks are detected here
// instead.
